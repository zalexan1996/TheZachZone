...
public class UpdateCharacterCommandHandler(IDatabaseService dbContext) : IRequestHandler<UpdateCharacterCommand, ZachZoneCommandResponse>
{
  ...
  {
    var record = await dbContext.Entity<Domain.Entities.PocketPersona.Character>().SingleOrDefaultAsync(x => x.Id == request.Id, cancellationToken);
    ...
    var game = await dbContext.Entity<Game>().SingleOrDefaultAsync(x => x.Id == record.Id, cancellationToken);
